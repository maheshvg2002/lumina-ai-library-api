...
from app.models.sql_models import Borrow, Review, Book, User, UserPreference
...
async def process_review_sentiment(review_id: int, review_text: str, db_gen):
    try:
...
def return_book(
    ...
    borrow_record = db.query(Borrow).filter(Borrow.id == borrow_id, Borrow.user_id == current_user.id).first()
...
def create_review(
    ...
    if not has_borrowed:
        raise HTTPException(status_code=403, detail="You must borrow a book before reviewing it.")
    ...
    background_tasks.add_task(process_review_sentiment, new_review.id, new_review.comment, get_db)
    return new_review
...
def get_ml_recommendations(
    ...
    borrowed_books = db.query(Borrow.book_id).filter(Borrow.user_id == current_user.id).all()
    ...
    explicit_prefs = db.query(UserPreference.topic_tag).filter(UserPreference.user_id == current_user.id).all()
    ...
    if borrowed_book_ids:
        other_books_query = db.query(Book).filter(Book.id.notin_(borrowed_book_ids)).all()
    else:
    ...
    scored_results = ml_engine.get_content_based_recommendations(
        user_liked_summaries=user_profile_text,
